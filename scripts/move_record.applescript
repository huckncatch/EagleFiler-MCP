...
on run argv
    -- argv[1]: library path, argv[2]: record GUID, argv[3]: target folder GUID
    set libPath to item 1 of argv
    set recGUID to item 2 of argv
    set targetGUID to item 3 of argv
    ...
    tell application "EagleFiler"
        ...
        try
            set targetFolder to first library record of theLib whose guid is targetGUID
        on error
            return my errResp("target folder not found: " & targetGUID)
        end try

        try
            set container of theRec to targetFolder
        on error errMsg
            return my errResp("Failed to move record: " & errMsg)
        end try
    end tell
